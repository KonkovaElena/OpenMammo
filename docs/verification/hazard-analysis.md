...
| HZ-005 | An unauthorized actor creates, reviews, exports, or delivers cases. | Static Bearer protection for `/api/v1/cases*`, request-aware envelopes, trusted actor identity derived from server-side auth config, and basic request hardening such as headers and rate limiting. | `tests/auth-protection.test.ts`, `tests/create-case.test.ts`, `tests/review-finalization.test.ts`, `tests/delivery-tracking.test.ts`, `tests/report-integrity.test.ts` | Partially reduced; current auth is shared-token machine identity, not user-grade authorization. |
...
1. Replace the shared-token boundary with stronger authentication and authorization, including token rotation and non-shared identities.
