...
| TR-008 | Protected case routes should require a valid Bearer credential when auth hardening is enabled, and actor identity should come from trusted config rather than caller-supplied headers. | Opt-in static Bearer route protection | `src/config.ts`, `src/index.ts`, `src/bootstrap.ts`, `src/application/createApp.ts` | `tests/auth-protection.test.ts`, `tests/config.test.ts` |

## Open Rows

The following trace rows do not exist yet and should be added before clinical deployment claims grow:

1. model governance and sidecar-output provenance
2. interoperability validation against live Orthanc and OHIF targets
3. human-factors validation and reviewer override behavior
