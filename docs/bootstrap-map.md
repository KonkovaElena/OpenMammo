...
- `src/bootstrap.ts`
  ...
  - wires the app with runtime dependencies, including the configured case repository, QC policy, baseline draft orchestrator path, and clinician review finalization use case

- `src/application/createApp.ts`
  - creates the Express app
  - sets correlation headers
  - exposes `/healthz`, `/readyz`, `/metrics`, `/api/v1/manifest`, `/api/v1/cases`, `/api/v1/cases/:caseId/review`, `/api/v1/cases/:caseId`, and `/api/v1/cases/:caseId/events`
